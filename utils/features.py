...
# heavily based on https://github.com/onanypoint/epfl-semester-project-biaxialnn/blob/master/utils/features.py
...
class FeatureBuilder(object):
    ...
    def note_input_form(self, note, state, time):
        ...
        def _build_context(state):
            ...
            for note, notestate in enumerate(state):
                if notestate[0] == 1:
                    pitchclass = (note + self.lower_bound) % 12
        ...
        part_prev_vicinity = list(itertools.chain.from_iterable(
            (_get_or_default(state[:, 0:2], note+i, [0, 0]) for i in range(-12, 13))))
    ...
    def note_state_matrix_to_input_form(self, statematrix):
        ...
        """
        inputform = [self.note_state_single_to_input_form(
